...
class Magento2(object):
	...
	def get_magento_stock(self):
		params = {
			"fields": 'items[product_id,qty]',
			"scopeId": 0,
			"qty":1000000000.0,
			'pageSize':1000000000
		}
		endpoint ='{rest_v1}/stockItems/lowStock'.format(rest_v1=self.rest_uri)
		return dict([(product.get('product_id'),product.get('qty')) for product in self._get_data(endpoint,params).get('data').get('items',[])])
