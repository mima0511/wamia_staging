...
class Importmagento2xProducts(models.TransientModel):
    ...
    def _import_magento2x_categories(self, channel_id, sdk, kwargs, obj):
        result = dict(
            mapping_ids=None,
            message=None
        )
        try:
            ...
            if feeds:
                if channel_id.auto_evaluate_feed:
                    result['mapping_ids'] = feeds.with_context(get_mapping_ids=True).import_items()
            else:
                message = "Category Feed creation failed!"
    ...
    @api.model
    def _magento2x_create_product_categories(self, channel_id, sdk, kwargs, category_ids):
        result = dict(
            res=False,
            import_res=False
        )
        ...
        if len(category_ids):
            ...
            result['categ_import'] = self._import_magento2x_categories(channel_id ,sdk, kwargs, obj)

        return result

    
    def get_magento2x_product_varinats_data(self, sdk, product_link_item):
    ...
    def get_magento2x_product_varinats(
            self, sdk, channel_id,
            product_data, extension_attributes,
            attributes_list, kwargs):
        res=[]
        ...
        for product_link_ids in chunks(
            extension_attributes.get('configurable_product_links'),api_record_limit):
            params = {
                "searchCriteria[filter_groups][0][filters][0][field]": 'entity_id',
            ...
            if debug:
                _logger.debug("=config-simple=%r =%r=%r "%(
            ...
            if product_link_data and product_link_data.get('items'):
                for product_link_item in product_link_data.get('items'):
                    product_item = self.get_magento2x_product_varinats_data(sdk,product_link_item)
                    if product_item:
                        vals = self.get_magento2x_product_vals(
                            sdk,channel_id,
                            product_item.get('id'),
                            product_item, kwargs,
    ...
    def get_magento2x_product_name_value(custom_attributes,product_data,
        ...
        products_custom_attributes = list(map(lambda i:(i.get('value'),i.get('attribute_code')),product_data.get('custom_attributes')))
        select_custom_attr = list(map(lambda i: i.get('attribute_code'),attributes_list))
        filtered = filter(lambda i:i[1] in select_custom_attr,products_custom_attributes)
        custom_attributes_value = [i[0] for i in list(filtered)]
        name_value_lists = []
                ...
                if str(value_index) in custom_attributes_value:
                    ...
                    if filter_attr:
                        ...
                        for option in attr_options:
                            if (option.get('value') not in [False,None,'']) \
                                and int(option.get('value').strip()) == value_index:
    ...
    def get_magento2x_product_vals(
            self, sdk, channel_id,
        ...
        custom_attributes=dict(
            IndexItems(
                items=product_data.get('custom_attributes'),
                skey='attribute_code'
                )
            )
        ...
        if extension_attributes and len(extension_attributes) \
            and extension_attributes.get('stock_item',{}).get('qty'):
            vals['qty_available'] = int(
                extension_attributes.get('stock_item').get('qty')
                )
        if 'qty_available' not in vals:
            vals['qty_available']=int(stock_dict.get(vals.get('store_id')))
    
        if type_id == 'configurable':
            feed_variants = self.get_magento2x_product_varinats(
                                sdk, channel_id, product_data,
            ...
            if trace_variant:
                ...
                vals['default_code'] = default_code
            if attributes_list:
                vals['name_value']=self.get_magento2x_product_name_value(
                                        custom_attributes,product_data,
                                        attributes_list,configurable_product_options)
        media=product_data.get('media_gallery_entries',[])
        vals['barcode'] =  extract_item(custom_attributes.get('barcode'),'value')
        if len(media):
            res_img = channel_id._magento2x_get_product_images_vals(
                        sdk, channel_id, media, product_id)
            if res_img.get('message'):
                kwargs['ext_msg'] = res_img.get('message') \
                    if not kwargs.get('ext_msg') \
                        else kwargs.get('ext_msg') + " " + res_img.get('message')
            else:
    ...
    def _magento2x_import_product(
        ...
        if category_ids:
            categ_import = self._magento2x_create_product_categories(
            ...
            if categ_import.get('import_res'):
                if categ_import.get('categ_import').get('mapping_ids'):
                    ...
                    kwargs.update(
                        ext_msg=categ_import.get('categ_import').get('message'),
                        categ_import=False
                    )
        vals = self.get_magento2x_product_vals(
    ...
    def magento2x_import_products(
        ...
        message = ''
        import_res = list()
        page_len = page_size = kwargs.get('page_size')
        debug = channel_id.debug == 'enable'
        ...
        products = fetch_data.get('data',{}).get('items',False) or {}
        
        total_count = fetch_data.get('total_count')
        if current_page==ceil(total_count/page_len) or total_count == 0:
            kwargs.update(page_size=10000)
            channel_id.if_cron_then_update_date(kwargs, self._name)
        ...
        if products:
            global stock_dict
            stock_dict=sdk.get_magento_stock()
            if debug:
                _logger.debug(
                    "@@@@=%r %r =%r==%r"%(
            ...
            for product in products:
                import_res.append(
                    self._magento2x_import_product(
                        sdk, channel_id,
                        product.get('id'), product,
                        kwargs, attributes_list=attributes_list))
        else:
            pass
        return dict(
            res=import_res,
    ...
    def returndata(data):
        a = data.get('variants',[])
        if not a:
            _logger.debug('~~~~~~~GIVEN CONF PRODUCT HAS NO CHILDREN~~~~~~~~~~%r~~~~~~~~~~~~~~~~~~~',data)
